...
if [[ -z "$XILINX_PATH" ]]; then
	if [[ -z "$XILINX" ]]; then
	  if  [[ -z "$XILINX_VIVADO" ]]; then
		echo "Path to Xilinx Vivado installation (\$XILINX_PATH) not set - terminating!"
		exit 1
	  else
	  ...
	  export XILINX_PATH=$XILINX/data
	fi
fi
...
# check if path to Mentor ModelSim installation is set
if [[ -z "$TCPA_PATH" ]]; then
  echo "Path to TCPA sources (\$TCPA_PATH) not set - terminating!"
  exit 1
fi
...
cd sim;
find . -mindepth 1 -maxdepth 1 -not -name "xilinx_lib" -exec rm -rf {} \; ;


## Compile unisim and stuff 

mkdir -p $XILINX_LIB_PATH/xilinx_lib
if [ -d $XILINX_LIB_PATH/xilinx_lib/unisims_ver ]; then 
	echo "Xilinx library already compiled"; else 
	echo "compile_simlib -directory ./xilinx_lib/ -family all -language all -library all -simulator questasim" > $XILINX_LIB_PATH/xilinx_lib/invasic_simlib.tcl; 
	vivado -mode batch -source $XILINX_LIB_PATH/xilinx_lib/invasic_simlib.tcl ; 
fi;
...
if [ -d $XILINX_LIB_PATH/xilinx_lib/unisims_ver ]; then 
	vmap secureip_ver $XILINX_LIB_PATH/xilinx_lib/secureip ; 
	vmap secureip $XILINX_LIB_PATH/xilinx_lib/secureip ; 
	vmap axi_bfm $XILINX_LIB_PATH/xilinx_lib/secureip ; 
	vmap unisims_ver $XILINX_LIB_PATH/xilinx_lib/unisims_ver ; 
	vmap unisim $XILINX_LIB_PATH/xilinx_lib/unisim ; 
	vmap unimacro_ver $XILINX_LIB_PATH/xilinx_lib/unimacro_ver ; 
	vmap unimacro $XILINX_LIB_PATH/xilinx_lib/unimacro ; 
	vmap simprim_ver $XILINX_LIB_PATH/xilinx_lib/simprims_ver ; 
	vmap unifast_ver $XILINX_LIB_PATH/xilinx_lib/unifast_ver ; 
	vmap unifast $XILINX_LIB_PATH/xilinx_lib/unifast_ver ; else 
...
vlib libs/cypress
vlib libs/hynix
vlib libs/wppa_instance_v1_01_a
...
vmap wppa_instance_v1_01_a libs/wppa_instance_v1_01_a
...
vcom -quiet -explicit -O0  -work wppa_instance_v1_01_a $TCPA_PATH/ARRAY/wppe_lib.vhd
vcom -quiet -explicit -O0  -work wppa_instance_v1_01_a $TCPA_PATH/ARRAY/default_lib.vhd
vcom -quiet -explicit -O0  -work wppa_instance_v1_01_a $TCPA_PATH/ARRAY/array_lib.vhd
vcom -quiet -explicit -O0  -work wppa_instance_v1_01_a $TCPA_PATH/ARRAY/type_lib.vhd
vcom -quiet -explicit -O0  -work wppa_instance_v1_01_a $TCPA_PATH/ARRAY/invasic_lib.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/reg_file.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/branch_control.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/wppe_multiplexer.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/mult_instr_decoder.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/logic_fu.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/connection.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/lut_fifo_memory.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/start_manager.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/data_path_unit.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/wppa.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/generic_loader.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/cpu_control.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/dpu_control.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/configuration_manager.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/flags_sel_unit.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/plb_controller.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/global_controller.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/adds_control.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/mux_sel_config_file.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/debug_entity_ctrl.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/shift_instr_decoder.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/div_control.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/wppe_icn_wrapper.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/logic_instr_decoder.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/config_memory.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/dpu_instr_decoder.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/domain_address_memory.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/bram_fifo_memory.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/flags_add_fu.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/mult_control.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/mux_2_1.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/control_regfile.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/cpu_instr_decoder.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/mult_fu.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/shift_fu.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/control_path_unit.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/shift_control.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/div_instr_decoder.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/debug_entity.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/mux_2_1_1bit.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/add_instr_decoder.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/fifo_common_clock.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/lut_feedback_register.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/wppa_top.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/instr_memory.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/wppe_2.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/logic_control.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/ARRAY/feedback_fifo.vhd
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/control_signal_generator.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/clock_generator.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/gc_rectangular_top.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/glbl.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/minmax_comparator.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/min_max_comparator_ic.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/reduction_and.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/my_CG_MOD.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/mux_one_hot_csg.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/drp_dcm.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/stride_selector_ripple.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/mux_ivar_select_csg.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/min_max_comparator_csg.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/my_or.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/mux_ivar_select.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/mux_conf_ack_select.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/minmax_comparator_matrix.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/comparator.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/reinitializer_rect.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/initializer.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/clock_generator_simple.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/control_fsm.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/loader_fsm.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/gc_apb_slave_mem.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/reduction_and_csg.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/my_or_csg.v
vlog -quiet +incdir+$TCPA_PATH/GLOBAL_CONTROLLER $TCPA_PATH/GLOBAL_CONTROLLER/mem_read_ctl.v
vcom -quiet -explicit -O0 $TCPA_PATH/GLOBAL_CONTROLLER_VHDL/gc_apb_slave_mem_wrapper.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/AG_BUFFER_type_lib.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/AG_Buffer_Wrapper.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/CSR.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/AG_Configurator.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/Extended_AND.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/ahb_slave_rbuffer.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/RBuffer_Configurator.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/Enable_Logic.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/GC_AG_glue.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/BRAM_Infered.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/BRAM_Wrapper_sim.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/OutputSelector.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/InputSelector.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/Scalar_Product_Simple.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/Memory.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/AG_MATRIX_Pipelined.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/Simple_Matrix_Mult.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/AHB_AG_Buffer_Wrapper.vhd
#vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/interrupt_generator.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/RBuffer.vhd
#vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/v2/AG_Matrix_Single_v2.vhd
#vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/v2/AG_MATRIX_Pipelined_v2.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/General_Multiplexer.vhd
#vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/RBuffer_hirq.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/RBUFFER/AG_Matrix_Single.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/TOP/reconfig_registers.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/TOP/top_hardware_interface.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/TOP/tcpa_config_mem.vhd
vcom -quiet -explicit -O0 $TCPA_PATH/TOP/TCPA_TOP_new.vhd

#vlog -quiet ../src/glbl.v

#VSIMOPT="-novopt -L unisims_ver -L secureip -do ../vsim_oip_wppe.do "
#vsim $VSIMOPT tb_oip_wppe 
